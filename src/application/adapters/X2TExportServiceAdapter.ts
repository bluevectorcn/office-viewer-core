...
import type { X2TExportService, X2TExportOptions } from '../use-cases/ExportDocumentUseCase';
...
export class X2TExportServiceAdapter implements X2TExportService {
  async exportWithX2T(
    source: Blob,
    format: ExportFormat,
    options?: X2TExportOptions
  ): Promise<Blob> {
    return await legacyExportWithX2T(source, format, options);
  }
}
